        ...
        public async Task<IActionResult> Bulk(BulkNotificationViewModel viewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Debug logging
                    Console.WriteLine($"🔍 DEBUG: SelectedRoleIds = {string.Join(", ", viewModel.SelectedRoleIds ?? new List<int>())}");
                    Console.WriteLine($"🔍 DEBUG: SendEmail = {viewModel.SendEmail}");
                    Console.WriteLine($"🔍 DEBUG: SendSMS = {viewModel.SendSMS}");

                    // Determine target users based on selected roles
                    if (viewModel.SelectedRoleIds != null && viewModel.SelectedRoleIds.Any())
                    {
                        // Get users by selected role IDs
                        userIds = await _context.Users
                            .Where(u => (viewModel.IncludeInactiveUsers || u.IsActive) &&
                                       viewModel.SelectedRoleIds.Contains(u.CustomRoleId ?? 0))
                            .Select(u => u.Id)
                            .ToListAsync();
                    ...
                    {
                        // Fallback: if no roles selected, don't send to anyone
                        userIds = new List<int>();
                        Console.WriteLine("🔍 DEBUG: No roles selected, userIds is empty");
                    }
